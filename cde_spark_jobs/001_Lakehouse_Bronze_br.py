...
# CREATE TABLE BRANCH
spark.sql("ALTER TABLE spark_catalog.HOL_DB_{0}.HIST_TRX_{0} DROP BRANCH IF EXISTS ing_branch".format(username))
...
trxBatchDf.write.format("iceberg").option("branch", "ing_branch").mode("append").save("SPARK_CATALOG.HOL_DB_{0}.HIST_TRX_{0}".format(username))
...
print(spark.read.option("branch", "ing_branch").format("iceberg").load("SPARK_CATALOG.HOL_DB_{0}.HIST_TRX_{0}".format(username)).count())
